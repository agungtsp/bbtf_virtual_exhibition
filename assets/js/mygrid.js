    ...
    function load_data(url){
        ...
        ajx = $.ajax({
            ...
            success	: function(msg){
                        ...
                        $(grid_id+' table tbody tr.footer td div .show_page').parent().prepend('<button type="button" class="btn reload tangan" title="Refresh"><i class="fa fa-refresh"></i></button> &nbsp;&nbsp;&nbsp;');
						...
						$(grid_id +' .reload').click(function(){
                            ...
                            $(grid_id +' .perpage').val(default_perpage);
                            $(grid_id).find('.cari').each(function(){
                                if(!$(this).hasClass('skip_clear')){
                                    $(this).val('');
                                }
                                
                            })
                            localStorage.removeItem('paging_session'+$(grid_id).attr('class').split(' ')[3]);
                            localStorage.removeItem('paging_session'+$(grid_id).attr('class').split(' ')[3]);
                            my_grid(order_id);
                        })
