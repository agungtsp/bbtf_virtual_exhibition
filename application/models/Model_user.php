    ...
    function __construct(){
	 ...
	 function user($where,$total='',$sidx='',$sord='',$mulai='',$end=''){
		...
		if ($total==1){
			...
			$sql	= "SELECT a.id_auth_user as id, a.id_auth_user_grup, a.is_delete,grup, a.userid, a.full_name,a.email,a.phone,a.id_ref_user_category,profil_mitra_id
						FROM auth_user a
						LEFT JOIN auth_user_grup b ON a.id_auth_user_grup = b.id_auth_user_grup";
			...
			foreach($dt as $dtx){
				if($dtx['is_delete']==0){
					...
					$data[$n][] 	= $dtx['email'];
					$data[$n][] 	= $dtx['id_ref_user_category'];
					++$n;
				}
			}
		}
