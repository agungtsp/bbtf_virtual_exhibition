...
class Model_log extends CI_Model{
    ...
    function export_to_excel($where,$is_single_row=0){
        $tableAs = "access_log a";
        $this->db->select('a.id, b.userid, b.full_name, b.id_auth_user, a.log_date, a.activity, ip');
        $this->db->join("auth_user b", "a.id_auth_user = b.id_auth_user",'left');

        if($is_single_row==1){
            return $this->db->get_where($tableAs,$where)->row_array();
        } 
        else {
            return $this->db->get_where($tableAs,$where)->result_array();
        }
    }
