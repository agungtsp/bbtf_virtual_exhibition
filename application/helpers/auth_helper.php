...
function auth_menu(){
	 ...
	 if($apps=='apps'){
		  ...
		  if ($user_sess){ //kalo ada session nya..
				...
				else if($ref_menu == 'login'){ //kalo ada session,tapi dihalaman login (controller login), redirect ke halaman home
		  ...
		  else if($ref_menu !='login' && $ref_menu !='register'){//kalo ga ada sessionya...
				redirect("apps/login");
		  }
	 }
}
