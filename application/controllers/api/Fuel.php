...
class Fuel extends REST_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('Fuel_model', 'model');
    ...
    function process_post(){
       ...
       try {
            ...
            $this->form_validation->set_rules('amount', '"Jumlah (liter)', 'required');
            $this->form_validation->set_rules('total_price', '"Harga Total', 'required');
            ...
            if($post['id']){
                ...
                foreach ($query as $key => $value) {
                    $data[] = array(
                        ...
                        iso_date_custom_format($value['purchase_date'], 'd M Y'),
                        '<a href="fuel-add.html?id='.md5plus($value['id']).'" title="Ubah Data" class="fa fa-pencil-alt tangan action-form-icon"></a>
