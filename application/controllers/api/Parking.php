...
class Parking extends REST_Controller {

    function __construct()
    {
        parent::__construct();
        $this->load->model('Parking_model', 'model');
    ...
    function process_post(){
       ...
       try {
            ...
            $this->form_validation->set_rules('position', '"Posisi Parkir"', 'required'); 
            $this->form_validation->set_rules('floor', '"Lantai Parkir"', 'required');
            $this->form_validation->set_rules('gps_point', '"Lokasi GPS', 'required');
            ...
            if($post['id']){
                ...
                foreach ($query as $key => $value) {
                    $data[] = array(
                        ...
                        $value['floor'],
                        $value['position'],
                        "Rp. " . number_format($value['parking_fee'], 0, ',', '.'),
                        iso_date_custom_format($value['create_date'], 'd M Y'),
                        '<a href="parking-add.html?id='.md5plus($value['id']).'" title="Ubah Data" class="fa fa-pencil-alt tangan action-form-icon"></a>
